    ...
    public class MemberService : IMemberService
    {
        ...
        async Task<(bool, IEnumerable<IdentityError>?)> IMemberService.EditUserAsync(UserEditViewModel request, string userName)
        {
            ...
            if (request.Picture != null && request.Picture.Length > 0)
            {
                ...
                var newPicturPath = Path.Combine(wwwrootFolder!.First(x => x.Name == "UserPictures").PhysicalPath!, randomFileName);
                using var stream = new FileStream(newPicturPath, FileMode.Create);
